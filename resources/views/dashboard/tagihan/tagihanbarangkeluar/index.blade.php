    ...
    <table class="table table-bordered" id="tableBarangKeluar">
        <thead class="table-light">
          ...
          <tr>
            ...
            <th scope="col" class="text-center">Jumlah Barang Diluar</th>
            <th></th>
          </tr>
        </thead>
        <tbody>
            ...
            <tr>
                <th scope="row" class="text-center">{{ $tagihan->firstItem() + $key }}</th>
                <td style="width: 10%">{{ date('d-m-Y', strtotime($tagih->updated_at)) }}</td>
                <td>{{ $tagih->KategoriPenjualan->kategori }}</td>
                <td>{{ $tagih->KategoriPenjualan->namakategoripenjualan}}</td>
                <td class="text-end">@currency($tagih->total_tagihan)</td>
                <td class="text-center">
                  <a href="/dashboard/tagihan/barangkeluar/{{ $tagih->id }}" class="btn btn-primary btn-sm" ><span data-feather="eye"></span></a>
          ...
          <tr>
              <th colspan="4" class="text-center">Total Barang Diluar</th>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="bayarModal{{ $tagih->id }}Label">Pembayaran pada {{ $tagih->KategoriPenjualan->namakategoripenjualan }}</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <h5>Tagihan : @currency($tagih->total_tagihan)</h5>
          <form action="/dashboard/tagihan/barangkeluar" method="post">
              @csrf
              <input type="hidden" name="id" value="{{ $tagih->id }}">
              <input type="hidden" name="slugsumber" value="{{ $tagih->slugmain }}">
              <div class="mb-3 mt-3">
                <label for="keterangan">Keterangan</label>
                <input type="text" name="keterangan" class="form-control" value="Pembayaran tagihan dari {{ $tagih->KategoriPenjualan->namakategoripenjualan }}" required>
              </div>
